...
def TelaInsumo(page: ft.Page):
    frame_brutton = ft.Container(
        content=ft.Row(
            [
                ft.Text("Insumo",size=24,color="#000000",weight=ft.FontWeight.BOLD),
                ft.ElevatedButton(text="Adicionar Insumo",on_click=lambda e: ModInsumo(page),bgcolor=Styles.color("bg_button"),color=Styles.color("white"),style=ft.ButtonStyle(shape=ft.RoundedRectangleBorder(radius=10),padding=ft.Padding(10, 5, 10, 5))),
            ],
    ...
    for i in range(10):
        frame = ft.Container(
            content=ft.Row(
                [
                    ft.TextButton(text=f"Insumo {i}",on_click=lambda e: print("Botão clicado"),style=ft.ButtonStyle(color="#000000",padding=ft.Padding(10, 5, 10, 5))),
